...
import { NodeTypeRegistry, builtInNodeTypes, type NodeTypeDefinition } from "./nodeTypes";
import {
  clamp,
  findHandle,
  oppositeSide,
...
export class FlowEngine {
  ...
  constructor(options: FlowEngineOptions = {}) {
    ...
    this.registry = new NodeTypeRegistry(
      options.includeBuiltInNodeTypes === false ? [] : builtInNodeTypes,
    );
  ...
  getSnapshot = (): FlowSnapshot => ({
    nodes: this.getState().nodes,
    edges: this.getState().edges,
  });
  ...
  on<K extends keyof FlowEngineEvents>(
    event: K,
    handler: (payload: FlowEngineEvents[K]) => void,
  ): () => void {
  ...
  addNode(input: NewNodeInput): FlowNode {
    ...
    if (this.getState().nodeLookup.has(node.id))
  ...
  private carryBend(
    ...
    sourceMove: XYPosition | undefined,
    targetMove: XYPosition | undefined,
  ): FlowEdge {
  ...
  paste(at?: XYPosition): FlowNode[] {
    ...
    const offset = at
      ? this.snap({ x: at.x - bounds.x, y: at.y - bounds.y })
      : { x: step, y: step };
  ...
  private selectedRects() {
    const s = this.getState();
    return s.nodes
      .filter((n) => s.selectedNodeIds.has(n.id))
      .map((n) => ({ id: n.id, rect: this.getNodeRect(n) }));
  }
  ...
  private insertCopies(source: FlowSnapshot, offset: XYPosition): FlowNode[] {
    ...
    copies.forEach((node) =>
      this.commit([...this.getNodes(), node], this.getEdges(), { type: "addNode", node }),
    );
    edgeCopies.forEach((edge) =>
      this.commit(this.getNodes(), [...this.getEdges(), edge], { type: "addEdge", edge }),
    );
    this.endInteraction();
    this.setSelection(
      copies.map((n) => n.id),
  ...
  removeEdges(ids: string[]): void {
    ...
    this.commit(
      s.nodes,
      s.edges.filter((e) => !remove.has(e.id)),
      { type: "removeEdges", ids },
    );
  }
  ...
  reverseEdge(id: string): boolean {
    ...
    const oldSourceSide = findHandle(
      this.registry.get(sourceNode.type),
      "source",
      edge.sourceHandle,
    )?.side;
    const oldTargetSide = findHandle(
      this.registry.get(targetNode.type),
      "target",
      edge.targetHandle,
    )?.side;
    ...
    this.commit(
      s.nodes,
      s.edges.map((e) => (e.id === id ? reversed : e)),
      { type: "updateEdge", edge: reversed },
    );
    return true;
  }
  ...
  insertNodeOnEdge(edgeId: string, type: string, at?: XYPosition): FlowNode | null {
    ...
    const first = this.addEdge(
      { source: edge.source, sourceHandle: edge.sourceHandle, target: node.id },
      { type: edge.type, label: edge.label, animated: edge.animated },
    );
    const second = this.addEdge(
      { source: node.id, target: edge.target, targetHandle: edge.targetHandle },
  ...
  addConnectedNode(fromId: string, side: HandleSide, type: string, gap = 80): FlowNode | null {
    ...
    const sourceHandle = this.registry
      .get(from.type)
      .handles.find((h) => h.kind === "source" && h.side === side);
  ...
  selectNode(id: string, additive = false): void {
    const s = this.getState();
    if (!additive) {
      if (s.selectedNodeIds.size === 1 && s.selectedNodeIds.has(id) && !s.selectedEdgeIds.size)
        return;
  ...
  selectInRect(rect: Rect, additive = false): void {
    ...
    this.setSelection(
      additive ? new Set([...s.selectedNodeIds, ...hits]) : hits,
      additive ? s.selectedEdgeIds : [],
    );
  }
  ...
  fitView(options: { padding?: number; nodeIds?: string[]; maxZoom?: number } = {}): void {
    ...
    const bounds = getBounds(
      s.nodes.filter((n) => !ids || ids.has(n.id)).map((n) => this.getNodeRect(n)),
    );
    if (!bounds) {
  ...
  startReconnect(edgeId: string, end: EdgeEnd, pointer: XYPosition): void {
    ...
    this.store.setState({
      connection: { from, pointer, candidate: null, valid: false, reconnecting: edgeId },
    });
  }
  ...
  findConnectionCandidate(pointer: XYPosition): HandleRef | null {
    ...
    const options = def.handles
      ...
      .filter(
        ({ h }) =>
          this.canConnect(
            this.connectionFor(conn.from, { nodeId: hovered!.id, handleId: h.id }),
            conn.reconnecting,
          ).valid,
      )
      .sort(
        (a, b) =>
          Math.hypot(a.p.x - pointer.x, a.p.y - pointer.y) -
          Math.hypot(b.p.x - pointer.x, b.p.y - pointer.y),
      );
  ...
  updateConnection(pointer: XYPosition): void {
    ...
    this.store.setState({
      connection: {
        ...conn,
        pointer,
        candidate: candidate?.handleId ? candidate : null,
        valid,
        reason,
      },
    });
  ...
  endConnection(): FlowEdge | null {
    ...
    this.commit(
      s.nodes,
      s.edges.map((e) => (e.id === edge.id ? updated : e)),
      { type: "updateEdge", edge: updated },
    );
    return updated;
  }
  ...
  validate(
    ...
    for (const issue of result.issues) {
      for (const id of issue.nodeIds ?? [])
        if (issueNodeIds.get(id) !== "error") issueNodeIds.set(id, issue.severity);
      for (const id of issue.edgeIds ?? [])
        if (issueEdgeIds.get(id) !== "error") issueEdgeIds.set(id, issue.severity);
    }
