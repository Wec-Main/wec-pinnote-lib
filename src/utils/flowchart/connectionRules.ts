import type { Connection, FlowEdge, FlowNode } from "../../types/flowchart.types";
import type { NodeTypeRegistry } from "./nodeTypes";
import { findHandle } from "./geometry";
...
export function checkConnection(conn: Connection, ctx: ConnectionContext): ConnectionCheckResult {
  ...
  if (!sourceHandle)
    return invalid(
      `"${source.data.label}" has no output${conn.sourceHandle ? ` "${conn.sourceHandle}"` : ""}`,
    );
  if (!targetHandle)
    return invalid(
      `"${target.data.label}" has no input${conn.targetHandle ? ` "${conn.targetHandle}"` : ""}`,
    );
  ...
  if (sourceDef.maxOutgoing !== undefined && outgoing.length >= sourceDef.maxOutgoing) {
    return invalid(
      sourceDef.maxOutgoing === 0
        ? `${sourceDef.label} nodes cannot have outgoing connections`
        : `${sourceDef.label} allows at most ${sourceDef.maxOutgoing} outgoing connections`,
    );
  }
  if (targetDef.maxIncoming !== undefined && incoming.length >= targetDef.maxIncoming) {
    return invalid(
      targetDef.maxIncoming === 0
        ? `${targetDef.label} nodes cannot have incoming connections`
        : `${targetDef.label} allows at most ${targetDef.maxIncoming} incoming connections`,
    );
  }
  if (sourceHandle.maxConnections !== undefined) {
    const used = outgoing.filter(
      (e) => (e.sourceHandle ?? findHandle(sourceDef, "source")?.id) === srcHandleId,
    ).length;
    if (used >= sourceHandle.maxConnections)
      return invalid(`Output "${sourceHandle.label ?? srcHandleId}" is already connected`);
  }
  if (targetHandle.maxConnections !== undefined) {
    const used = incoming.filter(
      (e) => (e.targetHandle ?? findHandle(targetDef, "target")?.id) === tgtHandleId,
    ).length;
    if (used >= targetHandle.maxConnections)
      return invalid(`Input "${targetHandle.label ?? tgtHandleId}" is already connected`);
  }
  return { valid: true };
}
