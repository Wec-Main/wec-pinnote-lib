import { createContext, useContext, useRef, useSyncExternalStore, type MutableRefObject } from 'react';
import type { FlowEngine, FlowState } from '../core/FlowEngine';
...
export function useFlowContext(): FlowContextValue {
  ...
  if (!ctx) throw new Error('Flow components must be rendered inside <FlowProvider> or <FlowEditor>');
  return ctx;
}
 ...
 */
export function useFlowState<T>(selector: (state: FlowState) => T, equalityFn: (a: T, b: T) => boolean = Object.is): T {
