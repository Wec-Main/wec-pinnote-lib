import type { FlowEdge, FlowNode, FlowSnapshot } from '../models/FlowTypes';
...
const roleOf = (ctx: ValidationContext, node: FlowNode) => ctx.registry.get(node.type).role ?? 'default';
...
export const startNodeRequired: ValidationRule = (ctx) => {
  ...
  if (starts.length === 0) return [{ code: 'start-required', severity: 'error', message: 'The flow needs a Start node' }];
  if (starts.length > 1) {
    return [{ code: 'multiple-starts', severity: 'warning', message: `The flow has ${starts.length} Start nodes`, nodeIds: starts.map((n) => n.id) }];
  }
  return [];
};

export const endNodeRequired: ValidationRule = (ctx) =>
  ctx.nodes.some((n) => roleOf(ctx, n) === 'end') ? [] : [{ code: 'end-required', severity: 'error', message: 'The flow needs an End node' }];

export const disconnectedNodes: ValidationRule = (ctx) =>
  ctx.nodes
    .filter((n) => !ctx.incoming.get(n.id)?.length && !ctx.outgoing.get(n.id)?.length)
    .map((n) => ({ code: 'disconnected', severity: 'error' as const, message: `${name(n)} is not connected to anything`, nodeIds: [n.id] }));
...
export const unreachableNodes: ValidationRule = (ctx) => {
  ...
  return ctx.nodes
    .filter((n) => !seen.has(n.id) && (ctx.incoming.get(n.id)?.length || ctx.outgoing.get(n.id)?.length))
    .map((n) => ({ code: 'unreachable', severity: 'warning' as const, message: `${name(n)} cannot be reached from Start`, nodeIds: [n.id] }));
};
...
export const deadEnds: ValidationRule = (ctx) =>
  ctx.nodes
    ...
    .filter((n) => ctx.incoming.get(n.id)?.length && !ctx.outgoing.get(n.id)?.length)
    .map((n) => ({ code: 'dead-end', severity: 'warning' as const, message: `${name(n)} has no outgoing connection`, nodeIds: [n.id] }));

export const invalidConnections: ValidationRule = (ctx) => {
  const issues: Omit<ValidationIssue, 'id'>[] = [];
  for (const edge of ctx.edges) {
    const result = checkConnection(edge, { nodeLookup: ctx.nodeLookup, edges: ctx.edges, registry: ctx.registry, ignoreEdgeId: edge.id });
    if (!result.valid) {
      issues.push({
...
export const decisionBranches: ValidationRule = (ctx) =>
  ctx.nodes
    ...
    .map((n) => ({ code: 'decision-branches', severity: 'warning' as const, message: `Decision ${name(n)} should have at least two outgoing branches`, nodeIds: [n.id] }));

export const unknownNodeTypes: ValidationRule = (ctx) =>
  ctx.nodes
    .filter((n) => !ctx.registry.has(n.type))
    .map((n) => ({ code: 'unknown-type', severity: 'warning' as const, message: `${name(n)} uses unknown node type "${n.type}"`, nodeIds: [n.id] }));
...
export function validateFlow(flow: FlowSnapshot, registry: NodeTypeRegistry, rules: ValidationRule[] = defaultValidationRules): ValidationResult {
  const ctx = buildContext(flow, registry);
  const issues = rules.flatMap((rule) => rule(ctx)).map((issue, i) => ({ ...issue, id: `${issue.code}-${i}` }));
