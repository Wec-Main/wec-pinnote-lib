...
const SelectionBox = memo(function SelectionBox() {
  ...
  return (
    <div
      ...
      style={{ left: rect.x * v.zoom + v.x, top: rect.y * v.zoom + v.y, width: rect.width * v.zoom, height: rect.height * v.zoom }}
    />
  );
});
...
const EmptyState = memo(function EmptyState() {
  ...
  return (
    <div className={styles.empty}>
      ...
      <div className={styles.emptyTitle}>{readOnly ? 'This flow is empty' : 'Start building your flow'}</div>
      {!readOnly && <div className={styles.emptyText}>Drag a node from the left panel onto the canvas, or click one to add it.</div>}
    </div>
  );
});

const ConnectionHint = memo(function ConnectionHint() {
  const reason = useFlowState((s) => (s.connection && !s.connection.valid ? s.connection.reason : undefined));
  return reason ? <div className={styles.hint}>{reason}</div> : null;
});
 ...
 */
export function FlowCanvas({ background = 'dots', showMiniMap = true, showControls = true, keyboardShortcuts = true, className, children }: FlowCanvasProps) {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      engine.zoomAt(Math.pow(2, -e.deltaY * unit * speed), clientToCanvas({ x: e.clientX, y: e.clientY }));
    };
  ...
  const onDrop = (e: React.DragEvent) => {
    ...
    const node = engine.addNode({
      type,
      position: engine.snap({ x: p.x - def.defaultSize.width / 2, y: p.y - def.defaultSize.height / 2 }),
    });
