...
export const MiniMap = memo(function MiniMap() {
  ...
  const rects = useMemo(
    () => nodes.map((n) => ({ id: n.id, rect: engine.getNodeRect(n), color: engine.getDefinition(n.type).color })),
    [engine, nodes],
  );
  ...
  const moveTo = useCallback(
    (clientX: number, clientY: number) => {
      ...
      if (!r) return;
      engine.centerOn({ x: vbX + ((clientX - r.left) / r.width) * vbW, y: vbY + ((clientY - r.top) / r.height) * vbH });
    },
  ...
  return (
    <div className={styles.minimap} onPointerDown={onPointerDown} onWheel={(e) => e.stopPropagation()} onDoubleClick={(e) => e.stopPropagation()} data-flow-overlay>
      <svg ref={svgRef} width={WIDTH} height={HEIGHT} viewBox={`${vbX} ${vbY} ${vbW} ${vbH}`}>
        ...
        <path className={styles.minimapMask} d={`${outer} ${inner}`} fillRule="evenodd" />
        <rect className={styles.minimapView} x={view.x} y={view.y} width={view.width} height={view.height} strokeWidth={1.5 * scale} />
      </svg>
    </div>
  );
});
