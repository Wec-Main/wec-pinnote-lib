...
import { snapToAlignment } from '../src/components/WecFlow/flowchart/utils/alignment';
...
describe('clipboard', () => {
  it('pastes copied nodes and their internal edges as new selected nodes', () => {
    ...
    expect(pasted.map((n) => n.position)).toEqual([{ x: 500, y: 500 }, { x: 500, y: 700 }]);
...
describe('snapToAlignment', () => {
  it('snaps to the nearest matching edge within the threshold and reports a guide', () => {
    const snap = snapToAlignment({ x: 103, y: 500, width: 50, height: 20 }, [{ x: 100, y: 0, width: 80, height: 40 }], 6);
  ...
  it('leaves the rect alone when nothing is close enough', () => {
    const snap = snapToAlignment({ x: 300, y: 300, width: 50, height: 20 }, [{ x: 0, y: 0, width: 80, height: 40 }], 6);
